let Cairn = {
  ...
  pile: function (name, sheet, pile = {}, props = {}) {
    if (!sheet) {
      sheet = name;
      name = '';
    }
    ...
    Object.keys(sheet).forEach(className => {
      ...
      Object.keys(sheet[className]).forEach(attribute => {
        ...
        if (attribute === 'props') {
          props[Cairn._makeName(name, className)] = value;
        } else if (isNestedElement(attribute, value)) {
          Cairn.pile(Cairn._makeName(name, className), { [attribute]: value }, pile, props);
        } else {
